        ...
        <Pressable onPress={handlePress}>
            <View style={{flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
                <View style={{ flexDirection: "row", gap: 11, alignItems: "center", paddingVertical: 6, width: "100%" }}>
                    ...
                    <View>
                        <Text style={{ fontSize: 16, fontWeight: "500", color: isCurrentTrack ? "green" : "#202020" }}>{item.title}</Text>
                        <Text style={{ fontSize: 12, color: "#646464" }}>{item.singer}</Text>
                    </View>
                </View>
                <View>{isCurrentTrack ? <Image source={require("../assets/eq.gif")} style={{ height: 20, width: 20 }} /> : null}</View>
            </View>
        </Pressable>
    );
};
